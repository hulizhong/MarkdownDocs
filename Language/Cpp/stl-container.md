...
size();  //返回元素个数；
resize(n, defaultVal=0);  //设置大小，即分配空间也创建对象。即修改capacity，也修改size.
	//n<当前size，只保留前n个元素，其后的对象全被销毁。
	//n>当前size，末尾添加元素，按默认值或传入值。
	//n>当前capacity，触发内存分配，当前的指针、引用、迭代器都将会失效！！rabin.
reserve(n); //设置容量。只修改capacity，不改变size.
